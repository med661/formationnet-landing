...
function TechItem({ title, icon }: { title: string; icon: React.ReactNode }) {
  return (
    <div title={title} className="group flex items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3 transition-all duration-300 hover:-translate-y-0.5 hover:border-[var(--accent)] hover:bg-[var(--card)] hover:shadow-md hover:shadow-indigo-500/5 focus-within:border-[var(--accent)] motion-reduce:transition-none">
      <span className="grid h-10 w-10 place-items-center rounded-md border border-[var(--border)] text-[var(--navbar-icon)] transition-colors duration-300 group-hover:border-[var(--accent)] group-hover:text-[var(--accent)]">{icon}</span>
...
export default function TechnologiesSection({ t }: { t: Translations }) {
  return (
    <section id="tech" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
      <SectionHeader title={t.technologies.title} subtitle={t.technologies.subtitle} />
